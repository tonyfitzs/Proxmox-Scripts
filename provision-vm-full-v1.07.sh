...
#
# CHANGELOG
#   1.07 - Fix silent exit when run via `bash <(curl ...)`.
#          Cause: `read -p` was reading from a piped stdin and failing instantly,
#          which combined with `set -e` + EXIT trap printed only "Cleanup complete".
#          Fix:  `read ... < /dev/tty` everywhere; removed problematic traps;
#                moved configuration to file scope so worker functions see it.
#                Added prox_run helper so the script works whether you run it
#                on the target node or via SSH from another node.
################################################################################
...
# When run on a Proxmox node directly, prox_run executes locally.
# When run from elsewhere, set PROX_SSH_KEY before calling.
PROX_SSH_USER="root"
...
# Run a command on a Proxmox node.
# If the current host's primary IP matches the target node, run locally.
# Otherwise SSH there.
prox_run() {
  local target_node="$1"
  shift
...
show_menu() {
  ...
  # When this script is invoked via `bash <(curl ...)`, stdin is the script
  # delivery channel, not your keyboard. /dev/tty is your actual terminal.
...
build_flippiq() {
  ...
  log_info "VM $FLIPPIQ_VM_ID created. Open the Proxmox console to install Debian."
  read -p "Press Enter after OS installation is complete (or Ctrl+C to skip cleanup): " _ < /dev/tty
